...
#define D_SP_COSIMU_MAJOR_VERSION        2;           /** 联仿接口主版本号 */
#define D_SP_COSIMU_MINOR_VERSION        0;           /** 联仿接口次版本号 */
...
#define D_SP_OBJECT_TYPE_SNOW_COVER            38
#define D_SP_OBJECT_TYPE_OIL_STAIN             39
...
#define D_SP_VEHICLE_LIGHT_DAYTIME_RUN_LAMP       0x00000100
#define D_SP_VEHICLE_LIGHT_POSITION_LAMP          0x00000200
...
#define D_SP_SYNC_CMD_FRAME_TRIGGER              0x0008
...
/** 地图文件中geoReference、offset标签内容 **/
typedef struct 
{
    char     au1GeoReference[D_SP_SIZE_FILE_PATH];
    double   u4Xoffset;
    double   u4Yoffset;
    double   u4Zoffset;
    double   u4HdgOffset;  
}S_SP_OPENDRIVE_HEADER;
...
typedef struct
{
    ...
    S_SP_OPENDRIVE_HEADER sOpenDriveHeader;
} S_SP_INIT_PARAM;
...
typedef struct
{
    ...
    float               u4State;
    float               u4CycleTime;
    S_SP_COORDINATE     sPos;                               /**< @link S_SP_COORDINATE */
    uint8_t             u1Status;                           /**< @link D_SP_TRLIGHT_STATUS */
    uint8_t             au1Spare[3];
} S_SP_TRAFFIC_LIGHT;
...
typedef struct
{
    uint32_t    u4EgoId;                /* 多主车情况下CM模块对应的主车ID */
} S_SP_EGO_ID;
...
typedef struct
{
    ...
    uint32_t        u4DataSize;
    uint32_t        u4FrameNo;
    uint8_t         u1MajorVersion;           /** @link D_SP_COSIMU_MAJOR_VERSION */
    uint8_t         u1MinorVersion;           /** @link D_SP_COSIMU_MINOR_VERSION */
...
typedef union
{
    ...
    S_SP_EGO_ID                    sEgoId;                         /**< @msgid D_SP_PKG_ID_EGO_ID                    */
} S_SP_MSG_UNION;
