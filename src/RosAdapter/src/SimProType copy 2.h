...
#define D_SP_OBJECT_CATEGORY_NONE           0    /**< no category defined           @version 0x0100 */
#define D_SP_OBJECT_CATEGORY_PLAYER         1    /**< category is player            @version 0x0100 */
#define D_SP_OBJECT_CATEGORY_SENSOR         2    /**< category is sensor            @version 0x0100 */
#define D_SP_OBJECT_CATEGORY_CAMERA         3    /**< category is camera            @version 0x0100 */
#define D_SP_OBJECT_CATEGORY_LIGHT_POINT    4    /**< category is light point       @version 0x0100 */
#define D_SP_OBJECT_CATEGORY_COMMON         5    /**< category is common object     @version 0x0100 */
#define D_SP_OBJECT_CATEGORY_OPENDRIVE      6    /**< category is OpenDRIVE object  @version 0x0110 */

/*  ------ object visibility flags ------*/
#define D_SP_OBJECT_VIS_FLAG_ALL                      0xffff  /**< all visibility flags set                                @version 0x0100 */
#define D_SP_OBJECT_VIS_FLAG_NONE                     0x0000  /**< no visibility flags set                                 @version 0x0100 */
#define D_SP_OBJECT_VIS_FLAG_GFX                      0x0001  /**< object is visible in graphics                           @version 0x0100 */
#define D_SP_OBJECT_VIS_FLAG_TRAFFIC                  0x0002  /**< object is visible for traffic                           @version 0x0100 */
#define D_SP_OBJECT_VIS_FLAG_RECORDER                 0x0004  /**< object is visible on data recorder                      @version 0x0110 */
...
typedef struct
{
	...
	double				u4IdealFreq;	// 理想传感器的频率，仅对于毫米波、激光、超声波有效
	uint32_t			u4ResL;			// 传感器横向分辨率，仅对相机有效
	uint32_t			u4ResV;			// 传感器垂直分辨率，仅对相机有效
	double				u4SenSizeL;		// 传感器长度，仅对相机有效
	double				u4SenSizeW;		// 传感器宽度，仅对相机有效
	double				u4LenFd;		// 传感器焦距，仅对相机有效
	double				u4DistCenL;		// 传感器横向畸变中心，仅对相机有效
	double				u4DistCenW;		// 传感器垂直畸变中心，仅对相机有效
	double				u4DistParK1;	// 传感器畸变参数K1，仅对相机有效
	double				u4DistParK2;	// 传感器畸变参数K2，仅对相机有效
	uint8_t             au1Spare[2];
} S_SP_SENSOR_INFO;
...
typedef struct
{
    ...
    double                          u4initSpeed;          /**主车的初始速度                  @unit m/s*/
    ...
    float                           u4SteerAngle;
    uint8_t	                        u1objQuantity;	    /**目标的总数	                  @unit _ */
    uint8_t	                        u1irc_id;	        /**主车同车道后方车辆id	            @unit _ */
    uint8_t	                        u1irc_left_id;	    /**主车同车道左后方车辆id	        @unit _ */
    uint8_t	                        u1irc_right_id;	    /**主车同车道右后方车辆id	        @unit _ */
    uint8_t                         u1cipv_id;            /**主车前方横向距离小于2m的车辆中纵向距离最近的车辆id   @unit _ */
    uint8_t                         u1lmc_id;             /**主车左侧车辆id                  @unit _ */
    uint8_t                         u1rmc_id;             /**主车右侧车辆id                  @unit _ */
    uint8_t                         u1isReachInitSpeed;   /**主车是否到达初始速度*/
} S_SP_MIL_EGO_STATE;
...
typedef struct
{
    ...
    float       u4Angle;
    uint8_t	    u1ObjClass;
    uint8_t     u1Flag_detected; 	    /**在主车前面赋值11，在主车后面赋值为16	   @unit _   */
    uint8_t	    u1Movable;    	    /**是否可移动  	             @unit _   */
    uint8_t	    u1Moving;     	    /**是否在移动              	 @unit _   */
    uint8_t     u1Obj_Age;            /**目标被检测到的帧数累加	   @unit _   */
...
{
    uint64_t            u8RoadId;
    double              curvVert;             /**< change of vertical curvature in lane center                            @unit 1/m2                             @version 0x0100 */
    float               u4StartDx;
    float               u4Width;
...
typedef struct
{
    uint32_t type; //type 0：无信息 1：我是收费站 2：支付卡信息 3：扣款通知 4：结束通信
}S_SP_RSU_VNFP_INFO;
...
typedef union
{
    ...
    S_SP_IMU_DATA                   sImuData;
    S_SP_GNSS_GGA                   sGnssGGA;
    S_SP_UWB_DATA                   sUwbData;
